...
public class PatchedTokensHelper {

    @SuppressWarnings("unused")
    private volatile TokenRequestState tokenRequestState;

    private static final AtomicReferenceFieldUpdater<PatchedTokensHelper, TokenRequestState> tokenRequestStateUpdater = AtomicReferenceFieldUpdater
            .newUpdater(PatchedTokensHelper.class, TokenRequestState.class, "tokenRequestState");
            ...
            if (currentState == null) {
                ...
                if (forceNewTokens || tokens.isAccessTokenExpired() || tokens.isAccessTokenWithinRefreshInterval()) {
                    newState = new TokenRequestState(
                            prepareUni((!forceNewTokens && tokens.getRefreshToken() != null
    ...
    private Uni<Tokens> prepareUni(Uni<Tokens> uni) {
        return uni.onItemOrFailure().invoke((tokens, throwable) -> {
            //we only have a single outstanding request
            //so we don't need to CAS
            if (tokens != null) {
                tokenRequestStateUpdater.set(this, new TokenRequestState(tokens));
            } else {
                tokenRequestStateUpdater.set(this, null);
            }
        })
        // prevent all subscriptions to trigger multiple times the HTTP request before the end of the first one
